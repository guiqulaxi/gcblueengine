...
public:
    NonCopyableClass(){}
    NonCopyableClass(const NonCopyableClass&) = delete;              // 删除拷贝构造函数
    NonCopyableClass& operator=(const NonCopyableClass&) = delete;   // 删除拷贝赋值操作符
    // 其他成员和方法
};
    class sqlite3_connection : NonCopyableClass {
	private:
    ...
    class sqlite3_transaction : public NonCopyableClass {
	private:
	...
	public:
		bool isStringError; ///< modified code, Dewitt
		...
		int GetNumberCols(); ///< added this, Dewitt
	};
    class sqlite3_command : NonCopyableClass {
    private:
