import pygcb
def CreateDBObject():
    dbObj=pygcb.tcMissileDBObject()
    dbObj.mzClass='3M54 Klub Alfa'
    dbObj.natoClass='SS-N-27 Sizzler'
    dbObj.mnModelType=5
    dbObj.mnType=64
    dbObj.cost=0.000000
    dbObj.weight_kg=1920.000000
    dbObj.volume_m3=3.000000
    dbObj.initialYear=1900.000000
    dbObj.finalYear=2999.000000
    dbObj.country=''
    dbObj.designation=''
    dbObj.imageList=''
    dbObj.iconFileName='mis/ico3M54.jpg'
    dbObj.mz3DModelFileName='mm-38.xml'
    dbObj.notes=''
    dbObj.length_m=1.000000
    dbObj.width_m=1.000000
    dbObj.height_m=1.000000
    dbObj.mfDamage=450.000000
    dbObj.damageModel='Pen200kg'
    dbObj.damageEffect='TestMissileDmg2'
    dbObj.launchSpeed_mps=0.000000
    dbObj.targetFlags=1
    dbObj.minLaunchAlt_m=-40.000000
    dbObj.maxLaunchAlt_m=34567.000000
    dbObj.minRange_km=2.000000
    dbObj.maxRange_km=220.000000
    dbObj.probNoFaults=0.850000
    dbObj.payloadClass='3M54 Warhead Section'
    dbObj.payloadQuantity=1
    dbObj.datalinkRange_km=0.000000
    dbObj.acceptsUserCommands=False
    dbObj.detonationRange_m=0.000000
    airDetectionDBObject=pygcb.tcAirDetectionDBObject()
    airDetectionDBObject.RCS_dBsm=-4.400000
    airDetectionDBObject.RCS_Model='MissileRCS1'
    airDetectionDBObject.opticalCrossSection_dBsm=4.810000
    airDetectionDBObject.irSignature_dB=0.000000
    airDetectionDBObject.IR_ModelA='IR_M_L_LP_M0.85'
    airDetectionDBObject.IR_ModelB='IR_M_L_LP_M0.85'
    airDetectionDBObject.IR_ModelC='IR_M_L_LP_M0.85'
    airDetectionDBObject.effectiveHeight_m=0.000000
    airDetectionDBObject.BindSignatureModels()
    dbObj.AddComponent(airDetectionDBObject)
    dbObj.mfDragArea_sm=22.400000
    dbObj.mfGmax=20.000000
    dbObj.mfMaxTurnRate_degps=109.300003
    dbObj.mfCdpsub=0.300000
    dbObj.mfCdptran=0.375000
    dbObj.mfCdpsup=0.250000
    dbObj.mfMcm=0.800000
    dbObj.mfMsupm=1.200000
    dbObj.mfBoostThrust_N=220000.000000
    dbObj.mfBoostTime_s=2.000000
    dbObj.mfSustThrust_N=21000.000000
    dbObj.mfSustTime_s=833.000000
    dbObj.mfShutdownSpeed_mps=200.000000
    dbObj.maSensorClass='ARGS-14E Seeker'
    dbObj.needsFireControl=False
    dbObj.acceptsWaypoints=False
    dbObj.fireAndForget=-1
    dbObj.isARM=-1
    dbObj.seekerFOV_rad=-1.000000
    dbObj.aczConstant_kts=1.286111
    dbObj.invMass_kg=0.000521
    dbObj.mnNumSegments=3
    dbObj.maFlightProfile=[pygcb.tsMissileFlightSegment()]*3
    dbObj.maFlightProfile[0].meAltitudeMode=pygcb.teAltitudeMode.AM_ASL
    dbObj.maFlightProfile[0].meGuidanceMode=pygcb.teGuidanceMode.GM_NAV
    dbObj.maFlightProfile[0].mfAltitude_m=15.000000
    dbObj.maFlightProfile[0].mfRange_km=33.000000
    dbObj.maFlightProfile[1].meAltitudeMode=pygcb.teAltitudeMode.AM_ASL
    dbObj.maFlightProfile[1].meGuidanceMode=pygcb.teGuidanceMode.GM_SENSOR1
    dbObj.maFlightProfile[1].mfAltitude_m=15.000000
    dbObj.maFlightProfile[1].mfRange_km=30.000099
    dbObj.maFlightProfile[2].meAltitudeMode=pygcb.teAltitudeMode.AM_ASL
    dbObj.maFlightProfile[2].meGuidanceMode=pygcb.teGuidanceMode.GM_DEPLOY
    dbObj.maFlightProfile[2].mfAltitude_m=15.000000
    dbObj.maFlightProfile[2].mfRange_km=30.000000
    dbObj.CalculateParams()
    return dbObj
