import pygcb
def CreateDBObject():
    dbObj=pygcb.tcMissileDBObject()
    dbObj.mzClass='S-35'
    dbObj.natoClass='SSC-1a Shaddock'
    dbObj.mnModelType=5
    dbObj.mnType=64
    dbObj.cost=0.000000
    dbObj.weight_kg=5400.000000
    dbObj.volume_m3=3.000000
    dbObj.initialYear=1960.000000
    dbObj.finalYear=2002.000000
    dbObj.country=''
    dbObj.designation=''
    dbObj.imageList=''
    dbObj.iconFileName='mis/ssm_unk.jpg'
    dbObj.mz3DModelFileName='mm-38.xml'
    dbObj.notes='(dewitt) changed from P-35 to S-35/SSC-1a since I think this is meant to be the surf-to-surf version'
    dbObj.length_m=1.000000
    dbObj.width_m=1.000000
    dbObj.height_m=1.000000
    dbObj.mfDamage=450.000000
    dbObj.damageModel='Blast1000kg'
    dbObj.damageEffect='TestMissileDmg'
    dbObj.launchSpeed_mps=0.000000
    dbObj.targetFlags=4
    dbObj.minLaunchAlt_m=-30.000000
    dbObj.maxLaunchAlt_m=34567.000000
    dbObj.minRange_km=2.000000
    dbObj.maxRange_km=500.000000
    dbObj.probNoFaults=0.850000
    dbObj.payloadClass=''
    dbObj.payloadQuantity=1
    dbObj.datalinkRange_km=0.000000
    dbObj.acceptsUserCommands=False
    dbObj.detonationRange_m=17.000000
    airDetectionDBObject=pygcb.tcAirDetectionDBObject()
    airDetectionDBObject.RCS_dBsm=3.900000
    airDetectionDBObject.RCS_Model='MissileRCS1'
    airDetectionDBObject.opticalCrossSection_dBsm=3.650000
    airDetectionDBObject.irSignature_dB=0.000000
    airDetectionDBObject.IR_ModelA='IR_M_VL_LP_M0.85'
    airDetectionDBObject.IR_ModelB='IR_M_VL_LP_M0.85'
    airDetectionDBObject.IR_ModelC='IR_M_VL_LP_M0.85'
    airDetectionDBObject.effectiveHeight_m=0.000000
    airDetectionDBObject.BindSignatureModels()
    dbObj.AddComponent(airDetectionDBObject)
    dbObj.mfDragArea_sm=19.570000
    dbObj.mfGmax=10.000000
    dbObj.mfMaxTurnRate_degps=106.191002
    dbObj.mfCdpsub=0.120000
    dbObj.mfCdptran=0.150000
    dbObj.mfCdpsup=0.100000
    dbObj.mfMcm=0.800000
    dbObj.mfMsupm=1.200000
    dbObj.mfBoostThrust_N=160000.000000
    dbObj.mfBoostTime_s=6.200000
    dbObj.mfSustThrust_N=21750.000000
    dbObj.mfSustTime_s=1410.000000
    dbObj.mfShutdownSpeed_mps=200.000000
    dbObj.maSensorClass='Generic Cruise Missile Seeker'
    dbObj.needsFireControl=False
    dbObj.acceptsWaypoints=False
    dbObj.fireAndForget=-1
    dbObj.isARM=-1
    dbObj.seekerFOV_rad=-1.000000
    dbObj.aczConstant_kts=2.572222
    dbObj.invMass_kg=0.000185
    dbObj.mnNumSegments=2
    dbObj.maFlightProfile=[pygcb.tsMissileFlightSegment()]*2
    dbObj.maFlightProfile[0].meAltitudeMode=pygcb.teAltitudeMode.AM_ASL
    dbObj.maFlightProfile[0].meGuidanceMode=pygcb.teGuidanceMode.GM_NAV
    dbObj.maFlightProfile[0].mfAltitude_m=200.000000
    dbObj.maFlightProfile[0].mfRange_km=1.500000
    dbObj.maFlightProfile[1].meAltitudeMode=pygcb.teAltitudeMode.AM_INTERCEPT
    dbObj.maFlightProfile[1].meGuidanceMode=pygcb.teGuidanceMode.GM_SENSOR1
    dbObj.maFlightProfile[1].mfAltitude_m=0.020000
    dbObj.maFlightProfile[1].mfRange_km=0.020000
    dbObj.CalculateParams()
    return dbObj
