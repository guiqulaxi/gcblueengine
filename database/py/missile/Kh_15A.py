import pygcb
def CreateDBObject():
    dbObj=pygcb.tcMissileDBObject()
    dbObj.mzClass='Kh-15A'
    dbObj.natoClass='AS-16 Kickback A'
    dbObj.mnModelType=5
    dbObj.mnType=64
    dbObj.cost=700000.000000
    dbObj.weight_kg=1200.000000
    dbObj.volume_m3=0.000000
    dbObj.initialYear=1980.000000
    dbObj.finalYear=2999.000000
    dbObj.country=''
    dbObj.designation=''
    dbObj.imageList=''
    dbObj.iconFileName='mis/icoKH-15A.jpg'
    dbObj.mz3DModelFileName='mm-38.xml'
    dbObj.notes='http://www.missilethreat.com/cruise/id.84/cruise_detail.asp  Range of 150 km from medium altitude, 100 km from low altitude. Mach5 at high altitude. Mach2 at low altitude'
    dbObj.length_m=1.000000
    dbObj.width_m=1.000000
    dbObj.height_m=1.000000
    dbObj.mfDamage=221.000000
    dbObj.damageModel='Pen250kg'
    dbObj.damageEffect='TestMissileDmg'
    dbObj.launchSpeed_mps=0.000000
    dbObj.targetFlags=5
    dbObj.minLaunchAlt_m=2600.000000
    dbObj.maxLaunchAlt_m=34567.000000
    dbObj.minRange_km=30.000000
    dbObj.maxRange_km=200.000000
    dbObj.probNoFaults=0.850000
    dbObj.payloadClass=''
    dbObj.payloadQuantity=1
    dbObj.datalinkRange_km=0.000000
    dbObj.acceptsUserCommands=False
    dbObj.detonationRange_m=0.000000
    airDetectionDBObject=pygcb.tcAirDetectionDBObject()
    airDetectionDBObject.RCS_dBsm=-5.800000
    airDetectionDBObject.RCS_Model='MissileRCS1'
    airDetectionDBObject.opticalCrossSection_dBsm=8.330000
    airDetectionDBObject.irSignature_dB=0.000000
    airDetectionDBObject.IR_ModelA='IR_M_L_R_M0.85'
    airDetectionDBObject.IR_ModelB='IR_M_L_R_M0.85'
    airDetectionDBObject.IR_ModelC='IR_M_L_R_M0.85'
    airDetectionDBObject.effectiveHeight_m=0.000000
    airDetectionDBObject.BindSignatureModels()
    dbObj.AddComponent(airDetectionDBObject)
    dbObj.mfDragArea_sm=0.010000
    dbObj.mfGmax=15.000000
    dbObj.mfMaxTurnRate_degps=49.700001
    dbObj.mfCdpsub=0.140000
    dbObj.mfCdptran=0.200000
    dbObj.mfCdpsup=0.900000
    dbObj.mfMcm=4.500000
    dbObj.mfMsupm=5.000000
    dbObj.mfBoostThrust_N=110000.000000
    dbObj.mfBoostTime_s=30.000000
    dbObj.mfSustThrust_N=20000.000000
    dbObj.mfSustTime_s=110.000000
    dbObj.mfShutdownSpeed_mps=200.000000
    dbObj.maSensorClass='Kh-15 Seeker'
    dbObj.needsFireControl=False
    dbObj.acceptsWaypoints=False
    dbObj.fireAndForget=-1
    dbObj.isARM=-1
    dbObj.seekerFOV_rad=-1.000000
    dbObj.aczConstant_kts=1.714815
    dbObj.invMass_kg=0.000833
    dbObj.mnNumSegments=3
    dbObj.maFlightProfile=[pygcb.tsMissileFlightSegment()]*3
    dbObj.maFlightProfile[0].meAltitudeMode=pygcb.teAltitudeMode.AM_ASL
    dbObj.maFlightProfile[0].meGuidanceMode=pygcb.teGuidanceMode.GM_NAV
    dbObj.maFlightProfile[0].mfAltitude_m=40000.000000
    dbObj.maFlightProfile[0].mfRange_km=55.000000
    dbObj.maFlightProfile[1].meAltitudeMode=pygcb.teAltitudeMode.AM_ASL
    dbObj.maFlightProfile[1].meGuidanceMode=pygcb.teGuidanceMode.GM_NAV
    dbObj.maFlightProfile[1].mfAltitude_m=10000.000000
    dbObj.maFlightProfile[1].mfRange_km=45.000000
    dbObj.maFlightProfile[2].meAltitudeMode=pygcb.teAltitudeMode.AM_INTERCEPT
    dbObj.maFlightProfile[2].meGuidanceMode=pygcb.teGuidanceMode.GM_SENSOR1
    dbObj.maFlightProfile[2].mfAltitude_m=200.000000
    dbObj.maFlightProfile[2].mfRange_km=0.100000
    dbObj.CalculateParams()
    return dbObj
