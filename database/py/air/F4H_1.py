import pygcb
def CreateDBObject():
    dbObj=pygcb.tcJetDBObject()
    dbObj.mzClass='F4H-1'
    dbObj.natoClass='F4H-1'
    dbObj.mnModelType=12
    dbObj.mnType=33
    dbObj.cost=15000000.000000
    dbObj.weight_kg=13104.000000
    dbObj.volume_m3=0.000000
    dbObj.initialYear=1958.000000
    dbObj.finalYear=2999.000000
    dbObj.country='USA'
    dbObj.designation='Fighter Multi'
    dbObj.imageList='f4e.jpg'
    dbObj.iconFileName='air/AdvicoF4H1.jpg'
    dbObj.mz3DModelFileName='f-4.xml'
    dbObj.notes=''
    dbObj.length_m=1.000000
    dbObj.width_m=1.000000
    dbObj.height_m=1.000000
    sensorPlatformDBObject=pygcb.tcSensorPlatformDBObject()
    sensorPlatformDBObject.sensorClass=['APQ-72','Eyeball 20/10','Eyeball 20/10d','Eyeball 20/10e','Eyeball 20/10f','Laser-10km']
    sensorPlatformDBObject.sensorAz=[0.000000,0.000000,0.000000,0.000000,0.000000,0.000000]
    dbObj.AddComponent(sensorPlatformDBObject)
    dbObj.mfMaxSpeed_kts=1370.000000
    dbObj.mfAccel_ktsps=12.890000
    dbObj.mfTurnRate_degps=14.000000
    dbObj.mfFuelCapacity_kg=6100.000000
    dbObj.mfFuelRate_kgps=4.700000
    dbObj.mfToughness=1.000000
    dbObj.damageEffect='F4H-1 durability'
    dbObj.mnNumLaunchers=5
    dbObj.maLauncherClass=['F-4B 1-9','F-4B 2-8','F-4B 3-7','F-4B 4-6','F-4B 5']
    dbObj.maMagazineClass=[]
    dbObj.magazineId=[]
    dbObj.launcherId=[0,1,2,3,4]
    dbObj.launcherName=['OW','IW','FF','AF','C']
    dbObj.launcherFOV_deg=[0.000000,0.000000,0.000000,0.000000,0.000000]
    dbObj.launcherAz_deg=[0.000000,0.000000,0.000000,0.000000,0.000000]
    dbObj.launcherEl_deg=[0.000000,0.000000,0.000000,0.000000,0.000000]
    dbObj.launcherFireControl=['APQ-72','APQ-72','APQ-72','APQ-72','APQ-72']
    dbObj.launcherFireControl2=['Laser-10km','Laser-10km','Laser-10km','Laser-10km','Laser-10km']
    dbObj.launcherIsReloadable=[False,False,False,False,False]
    airDetectionDBObject=pygcb.tcAirDetectionDBObject()
    airDetectionDBObject.RCS_dBsm=11.300000
    airDetectionDBObject.RCS_Model='AirRCS1'
    airDetectionDBObject.opticalCrossSection_dBsm=6.970000
    airDetectionDBObject.irSignature_dB=0.000000
    airDetectionDBObject.IR_ModelA='IR_FW_M_F_HP_2_M0.85'
    airDetectionDBObject.IR_ModelB='IR_FW_M_F_HP_2_M0.85'
    airDetectionDBObject.IR_ModelC='IR_FW_M_F_HP_2_M0.85'
    airDetectionDBObject.effectiveHeight_m=0.000000
    airDetectionDBObject.BindSignatureModels()
    dbObj.AddComponent(airDetectionDBObject)
    waterDetectionDBObject=pygcb.tcWaterDetectionDBObject()
    waterDetectionDBObject.TS=-999.000000
    waterDetectionDBObject.TS_Model='Default'
    waterDetectionDBObject.acousticModel='Default'
    waterDetectionDBObject.SL_Model='Default'
    waterDetectionDBObject.BindSignatureModels()
    dbObj.AddComponent(waterDetectionDBObject)
    dbObj.maxTakeoffWeight_kg=27075.000000
    dbObj.maxAltitude_m=18300.000000
    dbObj.climbRate_mps=210.000000
    dbObj.gmax=7.000000
    dbObj.minimumRunway_m=2500.000000
    dbObj.isCarrierCompatible=True
    dbObj.outFuelPods=0
    dbObj.fuelOut_kgps=0.000000
    dbObj.fuelIn_kgps=20.000000
    dbObj.maintenanceMin_s=3600.000000
    dbObj.maintenanceMax_s=10800.000000
    dbObj.militaryThrust_N=97006.000000
    dbObj.militaryThrustSpeedSlope=0.000000
    dbObj.mfAfterburnThrust_N=151294.000000
    dbObj.abThrustSpeedSlope=0.007000
    dbObj.mfAfterburnFuelRate_kgps=8.410000
    dbObj.mfCdpsub=0.690000
    dbObj.mfCdptran=2.400000
    dbObj.mfCdpsup=3.000000
    dbObj.mfMcm=0.800000
    dbObj.mfMsupm=1.200000
    dbObj.cruiseSpeed_mps=110.000000
    dbObj.stallSpeed_mps=60.000000
    dbObj.thrustTable=[0.900000,0.800000,0.700000,0.600000,0.500000,0.430000,0.200000,0.080000,0.010000,0.010000]
    dbObj.fuelEfficiencyTable=[1.000000,1.000000,1.000000,1.000000,1.000000,1.150000,1.700000,4.000000,60.000000,200.000000]
    dbObj.CalculateParams()
    return dbObj
