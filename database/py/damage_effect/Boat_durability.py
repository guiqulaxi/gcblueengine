import pygcb
def CreateDBObject():
    dbObj=pygcb.tcDamageEffect()
    dbObj.databaseClass='Boat durability'
    dbObj.blastEffect=[pygcb.DamagePoint()]*13
    dbObj.blastEffect[0].effectLevel=0.000000
    dbObj.blastEffect[0].damageFactor=0.000000
    dbObj.blastEffect[1].effectLevel=2.240000
    dbObj.blastEffect[1].damageFactor=0.000000
    dbObj.blastEffect[2].effectLevel=2.330000
    dbObj.blastEffect[2].damageFactor=0.100000
    dbObj.blastEffect[3].effectLevel=2.590000
    dbObj.blastEffect[3].damageFactor=0.200000
    dbObj.blastEffect[4].effectLevel=3.030000
    dbObj.blastEffect[4].damageFactor=0.300000
    dbObj.blastEffect[5].effectLevel=3.650000
    dbObj.blastEffect[5].damageFactor=0.400000
    dbObj.blastEffect[6].effectLevel=4.440000
    dbObj.blastEffect[6].damageFactor=0.500000
    dbObj.blastEffect[7].effectLevel=5.410000
    dbObj.blastEffect[7].damageFactor=0.600000
    dbObj.blastEffect[8].effectLevel=6.550000
    dbObj.blastEffect[8].damageFactor=0.700000
    dbObj.blastEffect[9].effectLevel=7.870000
    dbObj.blastEffect[9].damageFactor=0.800000
    dbObj.blastEffect[10].effectLevel=9.360000
    dbObj.blastEffect[10].damageFactor=0.900000
    dbObj.blastEffect[11].effectLevel=11.040000
    dbObj.blastEffect[11].damageFactor=1.000000
    dbObj.blastEffect[12].effectLevel=13.900000
    dbObj.blastEffect[12].damageFactor=2.000000
    dbObj.waterBlastEffect=[pygcb.DamagePoint()]*24
    dbObj.waterBlastEffect[0].effectLevel=0.000000
    dbObj.waterBlastEffect[0].damageFactor=0.000000
    dbObj.waterBlastEffect[1].effectLevel=59.037998
    dbObj.waterBlastEffect[1].damageFactor=0.048000
    dbObj.waterBlastEffect[2].effectLevel=67.002998
    dbObj.waterBlastEffect[2].damageFactor=0.050000
    dbObj.waterBlastEffect[3].effectLevel=76.004997
    dbObj.waterBlastEffect[3].damageFactor=0.053000
    dbObj.waterBlastEffect[4].effectLevel=86.179001
    dbObj.waterBlastEffect[4].damageFactor=0.056000
    dbObj.waterBlastEffect[5].effectLevel=97.672997
    dbObj.waterBlastEffect[5].damageFactor=0.059000
    dbObj.waterBlastEffect[6].effectLevel=110.658997
    dbObj.waterBlastEffect[6].damageFactor=0.062000
    dbObj.waterBlastEffect[7].effectLevel=125.332001
    dbObj.waterBlastEffect[7].damageFactor=0.067000
    dbObj.waterBlastEffect[8].effectLevel=141.916000
    dbObj.waterBlastEffect[8].damageFactor=0.071000
    dbObj.waterBlastEffect[9].effectLevel=160.673004
    dbObj.waterBlastEffect[9].damageFactor=0.077000
    dbObj.waterBlastEffect[10].effectLevel=181.910004
    dbObj.waterBlastEffect[10].damageFactor=0.083000
    dbObj.waterBlastEffect[11].effectLevel=205.994003
    dbObj.waterBlastEffect[11].damageFactor=0.091000
    dbObj.waterBlastEffect[12].effectLevel=233.373001
    dbObj.waterBlastEffect[12].damageFactor=0.100000
    dbObj.waterBlastEffect[13].effectLevel=264.610992
    dbObj.waterBlastEffect[13].damageFactor=0.111000
    dbObj.waterBlastEffect[14].effectLevel=300.438995
    dbObj.waterBlastEffect[14].damageFactor=0.125000
    dbObj.waterBlastEffect[15].effectLevel=341.855988
    dbObj.waterBlastEffect[15].damageFactor=0.143000
    dbObj.waterBlastEffect[16].effectLevel=390.303009
    dbObj.waterBlastEffect[16].damageFactor=0.167000
    dbObj.waterBlastEffect[17].effectLevel=448.029999
    dbObj.waterBlastEffect[17].damageFactor=0.200000
    dbObj.waterBlastEffect[18].effectLevel=518.921997
    dbObj.waterBlastEffect[18].damageFactor=0.250000
    dbObj.waterBlastEffect[19].effectLevel=610.687988
    dbObj.waterBlastEffect[19].damageFactor=0.333000
    dbObj.waterBlastEffect[20].effectLevel=742.341003
    dbObj.waterBlastEffect[20].damageFactor=0.500000
    dbObj.waterBlastEffect[21].effectLevel=984.846008
    dbObj.waterBlastEffect[21].damageFactor=1.000000
    dbObj.waterBlastEffect[22].effectLevel=1270.154053
    dbObj.waterBlastEffect[22].damageFactor=2.000000
    dbObj.waterBlastEffect[23].effectLevel=1622.754028
    dbObj.waterBlastEffect[23].damageFactor=4.000000
    dbObj.fragEffect=[pygcb.DamagePoint()]*24
    dbObj.fragEffect[0].effectLevel=0.000000
    dbObj.fragEffect[0].damageFactor=0.000000
    dbObj.fragEffect[1].effectLevel=1500.000000
    dbObj.fragEffect[1].damageFactor=0.000000
    dbObj.fragEffect[2].effectLevel=3000.000000
    dbObj.fragEffect[2].damageFactor=0.004410
    dbObj.fragEffect[3].effectLevel=6000.000000
    dbObj.fragEffect[3].damageFactor=0.006240
    dbObj.fragEffect[4].effectLevel=10000.000000
    dbObj.fragEffect[4].damageFactor=0.008060
    dbObj.fragEffect[5].effectLevel=30000.000000
    dbObj.fragEffect[5].damageFactor=0.013950
    dbObj.fragEffect[6].effectLevel=60000.000000
    dbObj.fragEffect[6].damageFactor=0.019730
    dbObj.fragEffect[7].effectLevel=100000.000000
    dbObj.fragEffect[7].damageFactor=0.025470
    dbObj.fragEffect[8].effectLevel=300000.000000
    dbObj.fragEffect[8].damageFactor=0.044120
    dbObj.fragEffect[9].effectLevel=600000.000000
    dbObj.fragEffect[9].damageFactor=0.062400
    dbObj.fragEffect[10].effectLevel=1000000.000000
    dbObj.fragEffect[10].damageFactor=0.080550
    dbObj.fragEffect[11].effectLevel=3000000.000000
    dbObj.fragEffect[11].damageFactor=0.139520
    dbObj.fragEffect[12].effectLevel=6000000.000000
    dbObj.fragEffect[12].damageFactor=0.197310
    dbObj.fragEffect[13].effectLevel=10000000.000000
    dbObj.fragEffect[13].damageFactor=0.254730
    dbObj.fragEffect[14].effectLevel=30000000.000000
    dbObj.fragEffect[14].damageFactor=0.441200
    dbObj.fragEffect[15].effectLevel=60000000.000000
    dbObj.fragEffect[15].damageFactor=0.623950
    dbObj.fragEffect[16].effectLevel=100000000.000000
    dbObj.fragEffect[16].damageFactor=0.805520
    dbObj.fragEffect[17].effectLevel=300000000.000000
    dbObj.fragEffect[17].damageFactor=1.395200
    dbObj.fragEffect[18].effectLevel=600000000.000000
    dbObj.fragEffect[18].damageFactor=1.973110
    dbObj.fragEffect[19].effectLevel=1000000000.000000
    dbObj.fragEffect[19].damageFactor=2.547270
    dbObj.fragEffect[20].effectLevel=3000000000.000000
    dbObj.fragEffect[20].damageFactor=4.412000
    dbObj.fragEffect[21].effectLevel=6000000000.000000
    dbObj.fragEffect[21].damageFactor=6.239520
    dbObj.fragEffect[22].effectLevel=10000000000.000000
    dbObj.fragEffect[22].damageFactor=8.055180
    dbObj.fragEffect[23].effectLevel=30000001024.000000
    dbObj.fragEffect[23].damageFactor=13.951980
    dbObj.radEffect=[pygcb.DamagePoint()]*6
    dbObj.radEffect[0].effectLevel=0.000000
    dbObj.radEffect[0].damageFactor=0.000000
    dbObj.radEffect[1].effectLevel=5000.000000
    dbObj.radEffect[1].damageFactor=0.000000
    dbObj.radEffect[2].effectLevel=25000.000000
    dbObj.radEffect[2].damageFactor=0.400000
    dbObj.radEffect[3].effectLevel=50000.000000
    dbObj.radEffect[3].damageFactor=2.000000
    dbObj.radEffect[4].effectLevel=125000.000000
    dbObj.radEffect[4].damageFactor=5.000000
    dbObj.radEffect[5].effectLevel=250000.000000
    dbObj.radEffect[5].damageFactor=20.000000
    dbObj.internalEffect=[pygcb.DamagePoint()]*7
    dbObj.internalEffect[0].effectLevel=0.000000
    dbObj.internalEffect[0].damageFactor=0.000000
    dbObj.internalEffect[1].effectLevel=1.000000
    dbObj.internalEffect[1].damageFactor=0.321500
    dbObj.internalEffect[2].effectLevel=2.000000
    dbObj.internalEffect[2].damageFactor=0.642620
    dbObj.internalEffect[3].effectLevel=4.000000
    dbObj.internalEffect[3].damageFactor=1.285170
    dbObj.internalEffect[4].effectLevel=8.000000
    dbObj.internalEffect[4].damageFactor=2.570820
    dbObj.internalEffect[5].effectLevel=16.000000
    dbObj.internalEffect[5].damageFactor=5.142790
    dbObj.internalEffect[6].effectLevel=32.000000
    dbObj.internalEffect[6].damageFactor=10.284280
    return dbObj
