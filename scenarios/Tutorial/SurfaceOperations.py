...
def CreateScenario(SM):
    ...
    unit.unitName = 'Hunter-1'
    unit.SetPosition(rally_lon - 0.2, rally_lat, 4500)
    unit.speed = 410
    unit.throttle = 0.8
    SM.AddUnitToAlliance(unit, alliance_b)
    UI = SM.GetUnitInterface(unit.unitName)
        
    AddWaypointOrderDeg(UI, rally_lon - 0.5, rally_lat+0.1)
    AddWaypointOrderDeg(UI, rally_lon - 0.2, rally_lat+0.1)
...
def AddGoals(SM, side_a, side_b):
    ...
    time_1 = SM.TimeGoal()
    time_1.SetPassTimeout(36000)

    SM.SetAllianceGoal(side_a, time_1)
    
    # alliance 2 goals
    time_2 = SM.TimeGoal()
    time_2.SetFailTimeout(36000)
