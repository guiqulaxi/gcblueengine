...
protected:
    ...
    std::string m_name;    //!< Layer's name
...
protected:
    std::string m_content; //!< string holding the coordinates to be drawn.
};
...
public:
    ...
    */
    mpFX(std::string name = wxEmptyString, int flags = mpALIGN_RIGHT);
...
public:
    ...
    */
    mpFY(std::string name = wxEmptyString, int flags = mpALIGN_TOP);
...
public:
    ...
    */
    mpFXY(std::string name = wxEmptyString, int flags = mpALIGN_NE);
...
public:
    /** @param name  Label
        @param flags Label alignment, pass one of #mpALIGN_BOTTOM, #mpALIGN_CENTER, #mpALIGN_TOP.
    */
    mpProfile(std::string name = wxEmptyString, int flags = mpALIGN_TOP);
...
public:
    ...
    mpScaleX(std::string name = wxT("X"), int flags = mpALIGN_CENTER, bool ticks = true, unsigned int type = mpX_NORMAL);
	...
	void SetLabelFormat(const std::string& format) { m_labelFormat = format; };

	/** Get X axis Label format (used for mpX_NORMAL draw mode).
	@return The format string */
	const std::string& SetLabelFormat() { return m_labelFormat; };
...
protected:
	...
	std::string m_labelFormat; //!< Format string used to print labels
...
public:
    /** @param name Label to plot by the ruler
        ...
        @param ticks Select ticks or grid. Give TRUE (default) for drawing axis ticks, FALSE for drawing the grid */
    mpScaleY(std::string name = wxT("Y"), int flags = mpALIGN_CENTER, bool ticks = true);
...
public:
    ...
    */
    mpLayer* GetLayerByName( const std::string &name);
...
//     wxCoord y2p(double y, bool drawOutside = true); // { return (wxCoord) ( (m_posY-y) * m_scaleY); }
    ...
    */
    ...
    bool SaveScreenshot(const std::string& filename, int type = wxBITMAP_TYPE_BMP, wxSize imageSize = wxDefaultSize, bool fit = false);
	...
	void SetLayerVisible(const std::string &name, bool viewable);
	
	/** Check whether a layer with given name is visible
		@param name The layer name
		@return layer visibility status */
	bool IsLayerVisible(const std::string &name );
...
public:
    /** @param name  Label
        @param flags Label alignment, pass one of #mpALIGN_NE, #mpALIGN_NW, #mpALIGN_SW, #mpALIGN_SE.
    */
    mpFXYVector(std::string name = wxEmptyString, int flags = mpALIGN_NE);
...
public:
    ...
    mpText(std::string name = wxT("Title"), int offsetx = 5, int offsety = 50);
...
public:
    ...
    mpCovarianceEllipse(
        ...
        double quantiles = 2,
        int    segments = 32,
        const std::string & layerName = wxT("") ) :
            m_cov_00(cov_00),
            m_cov_11(cov_11),
            m_cov_01(cov_01),
...
public:
    /** Default constructor.
      */
    mpPolygon( const std::string & layerName = wxT("") )
    {
        m_continuous = true;
        m_name = layerName;
    }
