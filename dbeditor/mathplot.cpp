...
mpFX::mpFX(std::string name, int flags)
{
    SetName(name);
    m_flags = flags;
    m_type = mpLAYER_PLOT;
...
mpFY::mpFY(std::string name, int flags)
{
    SetName(name);
    m_flags = flags;
    m_type = mpLAYER_PLOT;
...
mpFXY::mpFXY(std::string name, int flags)
{
    SetName(name);
    m_flags = flags;
    m_type = mpLAYER_PLOT;
...
mpProfile::mpProfile(std::string name, int flags)
{
    SetName(name);
    m_flags = flags;
    m_type = mpLAYER_PLOT;
...
mpScaleX::mpScaleX(std::string name, int flags, bool ticks, unsigned int type)
{
    SetName(name);
	...
	if (m_visible) {
		...
		wxLogMessage(wxT("mpScaleX::Plot: n: %f -> p = %d"), n, p);
			...
			if ((p >= startPx) && (p <= endPx)) {
				...
				if (m_labelType == mpX_NORMAL)
					s=strutil::format(fmt, n);
				else if (m_labelType == mpX_DATETIME) {
					time_t when = (time_t)n;
					struct tm tm = *localtime(&when);
					s=strutil::format(fmt, (double)tm.tm_year+1900, (double)tm.tm_mon+1, (double)tm.tm_mday, (double)tm.tm_hour, (double)tm.tm_min, (double)tm.tm_sec);
				} else if (m_labelType == mpX_DATE) {
					time_t when = (time_t)n;
					struct tm tm = *localtime(&when);
					s=strutil::format(fmt, (double)tm.tm_year+1900, (double)tm.tm_mon+1, (double)tm.tm_mday);
					...
					if (fmt.Len() == 20) // Format with hours has 11 chars
						s=strutil::format(fmt, sign*hh, mm, floor(ss));
					else
						s=strutil::format(fmt, sign*mm, ss);
				}
...
mpScaleY::mpScaleY(std::string name, int flags, bool ticks)
{
    SetName(name);
    SetFont( (wxFont&) *wxSMALL_FONT);
    SetPen( (wxPen&) *wxGREY_PEN);
...
mpLayer* mpWindow::GetLayerByName( const std::string &name)
{
...
bool mpWindow::SaveScreenshot(const std::string& filename, int type, wxSize imageSize, bool fit)
{
	int sizeX, sizeY;
	int bk_scrX, bk_scrY;
...
void mpWindow::SetLayerVisible(const std::string &name, bool viewable)
{
...
bool mpWindow::IsLayerVisible(const std::string &name )
{
	mpLayer* lx = GetLayerByName(name);
...
// Constructor
mpFXYVector::mpFXYVector(std::string name, int flags ) : mpFXY(name,flags)
{
    m_index = 0;
    m_minX  = -1;
    m_maxX  = 1;
...
*/
mpText::mpText( std::string name, int offsetx, int offsety )
{
    SetName(name);
