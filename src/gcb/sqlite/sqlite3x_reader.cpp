...
bool sqlite3_reader::read() {
	...
	isStringError = false; // modified code dewitt, reset string error after each call read()
...
std::string sqlite3_reader::getstring(int index) {
	...
	else // modified code dewitt
	{
#ifdef _DEBUG
		//isStringError = true;
		//fprintf(stderr, "sqlite3_reader::getstring -- Null string for column index (%d). Should be benign\n", index);
#endif
		return std::string("");
	}
}
...
* New code, Dewitt
*/
bool sqlite3_reader::getStringError() const
{
...
* New code, Dewitt
*/
int sqlite3_reader::GetNumberCols()
{
	if(this->cmd != 0)
	{
